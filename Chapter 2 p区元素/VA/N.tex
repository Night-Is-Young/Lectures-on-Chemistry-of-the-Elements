...
\begin{substance}[\ce{N2}]
    氮气,化学式为\ce{N2},是一种无色无味无臭的反磁性气体,熔点为$-210\tc$,沸点为$-195.8\tc$,难溶于水.
\end{substance}
...
\indent 一个有必要探讨的话题是固态氨中的氢键数目.事实上,与\ce{H2O}不同,固态氨中的每个\ce{N}原子均形成三组氢键,因此这一氢键事实上与冰中具有方向性和饱和性的氢键并不相同,更像是一种静电相互作用.这也导致了固态氨融化时实际上沉在液相的底部\footnote{参考数据:$198\K$时,液氨的密度为$0.731\text g\cdot\text{cm}^{-3}$;$193\K$时固态氨的密度为$0.820\text g\cdot\text{cm}^{-3}$.},这与冰和水恰恰相反.
...
\indent 现在,我们普遍认为其中含有金属离子\ce{M^x+}的氨配合物\ce{[M(NH3)_n]^x+}和分布在空穴中的自由电子\ce{e-}.这些空穴是排开\ce{NH3}分子形成的,因此会使得溶液密度大大减小.溶液显现蓝色正是由于自由电子的存在.\\
...
\begin{figure}[H]
    ...
    \subfigure[\ce{$\alpha$-Na2N2O3}的结构]{
        \begin{minipage}[b]{.45\linewidth}
            \centering\includegraphics{picture/alpha-N2O32-.eps}
        \end{minipage}
    }
    \subfigure[\ce{$\beta$-Na2N2O3}的结构]{
        \begin{minipage}[b]{.45\linewidth}
            \centering\includegraphics{picture/beta-N2O32-.eps}
        \end{minipage}
    }
